...
class Carabiner
{
    ...
    private function _asset($type, $dev_file, $prod_file = '', $combine, $minify, $media = 'screen', $group = 'main')
    {
        if ($type == 'css') :
            ...
            $this->css[$group][$media][$index]['minify'] = $minify; else :
    ...
    private function _display_js($group = 'main')
    {
        ...
        if ($this->dev) {
            ...
            foreach ($this->js[$group] as $ref):
                ...
                if (!$ref['combine']):
                    echo (isset($ref['prod'])) ? $this->_tag('js', $ref['prod']) : $this->_tag('js', $ref['dev']); elseif (!$ref['minify']):
            ...
            foreach ($this->js[$group] as $ref):
                ...
                if (!$ref['combine']):
                    echo (isset($ref['prod'])) ? $this->_tag('js', $ref['prod']) : $this->_tag('js', $ref['dev']); else:
        ...
        if ($this->dev) { // we're in a development environment
            ...
            foreach ($this->css[$group] as $media => $refs):
                ...
                foreach ($refs as $ref):
                    ...
                    if (!$ref['combine']):
                        echo (isset($ref['prod'])) ? $this->_tag('css', $ref['prod'], $media) : $this->_tag('css', $ref['dev'], $media); elseif (!$ref['minify']):
                        $files[] = (isset($ref['prod'])) ? array('prod'=> $ref['prod'], 'dev'=> $ref['dev'], 'minify'=> $ref['minify']) : array('dev'=> $ref['dev'], 'minify'=> $ref['minify']);
                    else:
                        $files[] = (isset($ref['prod'])) ? array('prod'=> $ref['prod'], 'dev'=> $ref['dev']) : array('dev'=> $ref['dev']);
                    endif;
            ...
            foreach ($this->css[$group] as $media => $refs):
                ...
                foreach ($refs as $ref):
                    ...
                    if ($ref['combine'] == FALSE):
                        echo (isset($ref['prod'])) ? $this->_tag('css', $ref['prod'], $media) : $this->_tag('css', $ref['dev'], $media); else:
                        $files[] = (isset($ref['prod'])) ? array('prod'=> $ref['prod'], 'dev'=> $ref['dev'], 'minify'=> FALSE) : array('dev'=> $ref['dev'], 'minify'=> FALSE);
                    endif;
    ...
    private function _get_contents($ref)
    {
        ...
        if ($this->isURL($ref) && (ini_get('allow_url_fopen') == 0 || $this->force_curl)):
            ...
            $this->_load('curl');
            $contents = $this->CI->curl->simple_get($ref); else:
            ...
            case 'css':
                ...
                foreach ($files as $file) {
                    ...
                    if (($ext == $flag) && $fl >= 42 && (filemtime($this->cache_path . $file) < $before)) {
                        ...
                        if ($success) : log_message('debug', 'Carabiner: Cache file ' . $file . ' was removed from ' . $this->cache_path); else : log_message('error', 'Carabiner: There was an error removing cache file ' . $file . ' from ' . $this->cache_path);
                        endif;
